...
Focal_Comm_vol <- CITES_TRUE %>% 
  ...
  Focal_NonComm_vol <- CITES_TRUE %>% 
    filter(Class %in% focal_class, Year %in% 2000:2020,
           ...
           Purpose != "T") %>%
    group_by_at(Sum_group) %>%
    reframe(NonComm_vol = sum(Quantity))
  
  Focal_Unknown_vol <- CITES_TRUE %>% 
    filter(Class %in% focal_class, Year %in% 2000:2020,
           ## R added to align with UNEP
           Source %in% c("C", "D", "F", "R"), 
           ## Focus on ER trade in number of individuals
...
#### Historic sources ####  
  ...
  Historic_source <- CITES_TRUE %>% 
    ...
    reframe(Year_F1 = paste(Year, collapse=', '))
  
#### Contrast reporter Sources ####
  
  cat(paste0("Compiling contrasting ", contrast_reporter, " reported trade sources\n" ))
  ...
  reporter_contrast_sum <- function(data = CITES_TRUE, codes = c("C", "D", "F")) {
    ...
    CITES_TRUE %>% 
      filter(Class %in% focal_class, Year %in% 2000:2020,
             ...
             Appendix != "N") %>%
      group_by_at(Sum_group) %>% 
      reframe(!!paste0(lab, "_Vol_", contrast_reporter, "R") := sum(Quantity)) %>%
      filter(!grepl("spp", Taxon), !grepl("hybrid", Taxon)) %>%
      ungroup()
    }
...
#### Contrasting reporter purposes ####
  cat(paste0("Compiling contrasting ", contrast_reporter, " reported trade purposes\n" ))
...
#### Historic source records ####
  ...
  focal_history <- function(data = CITES_TRUE, codes = c("C", "D", "F")) {
         ...
         CITES_Focal_Capt %>% 
            ...
            reframe(Year = seq(from = Year - 5, to = Year, length.out = 6)) %>%
...
#### Collating the data ####
  ...
  Full_data_to_check <- CITES_Focal_Capt %>%
    ...
    ## add the unknown vol
    left_join(Focal_Unknown_vol, by = Sum_group) %>%
    mutate(NA_vol = if_else(is.na(NA_vol), 0, NA_vol)) %>%
...
captive_assess <- function(data = data, focal_reporter = "E", Class_for_traits = "Aves") 
  ...
  data2 <- data %>% group_by(ROW_ID) %>% mutate(
    ...
    Check_1 = 
      Vol > 2*mean(c(Year_5_vol_Capt + Year_5_vol_Ranch, 
    ...
    ## mean of the last 5 years trade?
    Check_2 = 
      Vol > 4*mean(c(Year_5_vol_Capt + Year_5_vol_Ranch, 
                     Year_4_vol_Capt + Year_4_vol_Ranch,
                     Year_3_vol_Capt + Year_3_vol_Ranch,
                     Year_2_vol_Capt + Year_2_vol_Ranch,
                     Year_1_vol_Capt + Year_1_vol_Ranch)),
    ...
    Check_4 = case_when(
      Check_1 == TRUE & 
        ...
        abs((Vol - mean(c(Year_5_vol_Capt + Year_5_vol_Ranch, 
                          Year_4_vol_Capt + Year_4_vol_Ranch,
                          Year_3_vol_Capt + Year_3_vol_Ranch,
                          Year_2_vol_Capt + Year_2_vol_Ranch,
                          Year_1_vol_Capt + Year_1_vol_Ranch))) +
    ...
    mutate(Check_8 = ifelse(Check_7 == FALSE, FALSE, Check_8),
           ...
           ## of 0 / 0 in checks 10, 11 and 12, this is a catch all for those instances
           ## records automatically pass as if there is no reported trade there 
           ## cannot be source switching.
           Check_10 = ifelse((Capt_Vol_Contrast + Wild_Vol_Contrast) == 0,
                            FALSE, Check_10),
...
if(Class_for_traits == "Aves"){
  ...
  ## 14 - Score out of 4 for how "slow" life history traits are
  data2 <- data2 %>% group_by(ROW_ID) %>% mutate(
    Check_14 = (Adult_survival_Perc <= .25) + 
